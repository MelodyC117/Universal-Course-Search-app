...
try {
    ...
    $topic = "%".$str2."%";
    $tool = "%".$str3."%";
    $sql = "SELECT C.Course_ID, Course_Num, Course_Name, Course_Link, Focus_Name, Topic, Tool_Name
            FROM COURSE AS C
            JOIN FOCUS AS F
            ON C.Focus_ID = F.Focus_ID
            JOIN TOPIC AS T
            ON C.Course_ID = T.Course_ID
            JOIN TOOLS AS O
            ON C.Course_ID = O.Course_ID
            WHERE Focus_Name LIKE '$focus' AND Topic LIKE '$topic' AND Tool_Name LIKE '$tool'";
...
<html>
    ...
    <div id="wrap">
      <!-- go back button -->
      <div class="go_back">
        <a href="interface.html"><button type="button" id="bu"> GO BACK </button></a>
      </div>
            ...
            <table style="width: 100%;background-color: black; opacity: 0.55; font-style:normal; margin-top: -10px; color: white;" border="1" cellspacing="5" cellpadding="5">
                <thead>
                    <tr>
                      ...
                      <th align="left">Course Link</th>
                      <th align="left">Focus Name</th>
                      <th align="left">Topic</th>
                      <th align="left">Tool_Name</th>
                ...
                <tbody>
                    <?php while ($row = $q->fetch()): ?>
                        <tr>
                            <td align="left"><?php echo htmlspecialchars($row['Course_ID']); ?></td>
                            <td align="left"><?php echo htmlspecialchars($row['Course_Num']); ?></td>
                            <td align="left"><?php echo htmlspecialchars($row['Course_Name']); ?></td>
                            <td><a href="<?php echo $row['Course_Link']; ?>"><?php echo $row['Course_Link']; ?></a></td>
                            <td align="left"><?php echo htmlspecialchars($row['Focus_Name']); ?></td>
                            <td align="left"><?php echo htmlspecialchars($row['Topic']); ?></td>
                            <td align="left"><?php echo htmlspecialchars($row['Tool_Name']); ?></td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
